...
/mob/living/carbon/human/pl_effects()
	...
	if(vsc.plc.SKIN_BURNS)
		if(!pl_head_protected() || !pl_suit_protected())
			burn_skin(0.75)
			if(prob(20)) src << SPAN_DANG("Your skin burns!")
			updatehealth()
	...
	if(vsc.plc.GENETIC_CORRUPTION)
		if(rand(1,10000) < vsc.plc.GENETIC_CORRUPTION)
			randmutb(src)
			src << SPAN_DANG("High levels of toxins cause you to spontaneously mutate!")


/mob/living/carbon/human/proc/burn_eyes()
	var/obj/item/organ/internal/eyes/E = internal_organs_by_name[O_EYES]
	if(E)
		if(prob(20)) src << SPAN_DANG("Your eyes burn!")
		E.damage += 2.5
		eye_blurry = min(eye_blurry+1.5,50)
		if (prob(max(0,E.damage - 15) + 1) &&!eye_blind)
			src << SPAN_DANG("You are blinded!")
			eye_blind += 20

	...
	for(var/mob/living/carbon/human/M in orange(10, user))
		spawn()
			if(prob(50))
				...
				if(prob(25))
					M.Stun(rand(5,10))
				M << SPAN_DANG("You feel a tremendous, paralyzing wave flood your mind.")

			else
				M << SPAN_DANG("You feel a sudden, electric jolt travel through your head.")

	playsound(src.loc, 'sound/misc/interference.ogg', 50, 1)
	user << SPAN_NOTE("You trigger [src].")
	times_used += 1
	if(times_used >= max_uses)

...
/obj/structure/proc/can_climb(var/mob/living/user, post_climb_check=0)
	...
	if (!user.Adjacent(src))
		user << SPAN_DANG("You can't climb there, the way is blocked.")
		return 0
...
/obj/structure/proc/structure_shaken()
	for(var/mob/living/M in climbers)
		M.Weaken(1)
		M << SPAN_DANG("You topple as you are shaken off \the [src]!")
		climbers.Cut(1,2)
	...
	for(var/mob/living/M in get_turf(src))
		...
		M.Weaken(3)
		M << SPAN_DANG("You topple as \the [src] moves under you!")
		...
		if(prob(25))
			...
			if(affecting)
				M << SPAN_DANG("You land heavily on your [affecting.name]!")
...
/obj/structure/attack_generic(var/mob/user, var/damage, var/attack_verb, var/wallbreaker)
	if(!breakable || !damage || !wallbreaker)
		return 0
	visible_message(SPAN_DANG("[user] [attack_verb] the [src] apart!"))

...
/obj/machinery/door/attack_generic(var/mob/user, var/damage)
	if(damage >= 10)
		visible_message(SPAN_DANG("\The [user] smashes into the [src]!"))
		take_damage(damage)
	else
...
/obj/machinery/door/bullet_act(var/obj/item/projectile/Proj)
	...
	if (Proj.damage > 90)
		destroy_hits--
		if (destroy_hits <= 0)
			visible_message(SPAN_DANG("\The [src.name] disintegrates!"))
...
/obj/machinery/door/hitby(AM as mob|obj, var/speed=5)
	...
	..()
	visible_message(SPAN_DANG("[src.name] was hit by [AM]."))
	var/tforce = 0
	if(ismob(AM))
		tforce = 5 * (speed/5)
		var/mob/living/M = AM
	...
	if(src.density && istype(I, /obj/item/weapon) && user.a_intent == I_HURT && !istype(I, /obj/item/weapon/card))
		...
		if(W.damtype == BRUTE || W.damtype == BURN)
			...
			if(W.force < min_force)
				user.visible_message(SPAN_DANG("\The [user] hits \the [src] with \the [W] with no visible effect."))
			else
				user.visible_message(SPAN_DANG("\The [user] forcefully strikes \the [src] with \the [W]!"))
				playsound(src.loc, hitsound, 100, 1)
				take_damage(W.force)
		return

...
/obj/machinery/power/singularity_beacon/proc/Activate(mob/user = null)
	if(surplus() < 1500)
		if(user) user << SPAN_NOTE("The connected wire doesn't have enough current.")
		return
	...
	machines |= src
	if(user)
		user << SPAN_NOTE("You activate the beacon.")
...
/obj/machinery/power/singularity_beacon/proc/Deactivate(mob/user = null)
	...
	active = 0
	if(user)
		user << SPAN_NOTE("You deactivate the beacon.")
...
/obj/machinery/power/singularity_beacon/attack_hand(var/mob/user as mob)
	if(anchored)
		...
		user << SPAN_DANG("You need to screw the beacon to the floor first!")
		return
...
/obj/machinery/power/singularity_beacon/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/screwdriver))
		if(active)
			user << SPAN_DANG("You need to deactivate the beacon first!")
			return

		if(anchored)
			anchored = 0
			user << SPAN_NOTE("You unscrew the beacon from the floor.")
			...
			if(!connect_to_network())
				...
				return
			anchored = 1
			user << SPAN_NOTE("You screw the beacon to the floor and attach the cable.")
			return
	..()
	return

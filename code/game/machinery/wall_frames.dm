...
/obj/item/frame/proc/try_build(turf/on_wall)
	...
	if (!istype(loc, /turf/simulated/floor))
		usr << SPAN_DANG("\The [src] Alarm cannot be placed on this spot.")
		return
	if (A.requires_power == 0 || A.name == "Space")
		usr << SPAN_DANG("\The [src] Alarm cannot be placed in this area.")
		return

	if(gotwallitem(loc, ndir))
		usr << SPAN_DANG("There's already an item on this wall!")
		return

...
/datum/antagonist/proc/create_nuke(var/atom/paper_spawn_loc, var/datum/mind/code_owner)
	...
	if(code)
		...
		message_admins(SPAN_DANG("Could not spawn nuclear bomb. Contact a developer."))
		return
...
/datum/antagonist/proc/greet(var/datum/mind/player)
	...
	// Basic intro text.
	player.current << SPAN_DANG("<font size=3>You are a [role_text]!</font>")
	if(leader_welcome_text && player == leader)
		player.current << SPAN_NOTE(leader_welcome_text)
	else
		player.current << SPAN_NOTE(welcome_text)

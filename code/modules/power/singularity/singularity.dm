...
/obj/singularity/proc/expand(var/force_size = 0, var/growing = 1)
	...
	switch (temp_allowed_size)
		if (STAGE_ONE)
			...
			if(chained)
				overlays = "chain_s1"
			visible_message(SPAN_NOTE("The singularity has shrunk to a rather pitiful size."))
		if (STAGE_TWO) //1 to 3 does not check for the turfs if you put the gens right next to a 1x1 then its going to eat them.
			...
			if(chained)
				overlays = "chain_s3"
			if(growing)
				visible_message(SPAN_NOTE("The singularity noticeably grows in size."))
			else
				visible_message(SPAN_NOTE("The singularity has shrunk to a less powerful size."))
		if (STAGE_THREE)
			if ((check_turfs_in(1, 2)) && (check_turfs_in(2, 2)) && (check_turfs_in(4, 2)) && (check_turfs_in(8, 2)))
				...
				if(chained)
					overlays = "chain_s5"
				if(growing)
					visible_message(SPAN_NOTE("The singularity expands to a reasonable size."))
				else
					visible_message(SPAN_NOTE("The singularity has returned to a safe size."))
		if(STAGE_FOUR)
			if ((check_turfs_in(1, 3)) && (check_turfs_in(2, 3)) && (check_turfs_in(4, 3)) && (check_turfs_in(8, 3)))
				...
				if(growing)
					...
					visible_message(SPAN_NOTE("Miraculously, the singularity reduces in size, and can be contained."))
		if(STAGE_FIVE) //This one also lacks a check for gens because it eats everything.
			...
			if(chained)
				overlays = "chain_s9"
			if(growing)
				visible_message(SPAN_DANG("<font size='2'>The singularity has grown out of control!</font>"))
			else
				visible_message(SPAN_WARN("The singularity miraculously reduces in size and loses its supermatter properties."))
		...
		if(M.stat == CONSCIOUS)
			if (ishuman(M))
				...
				if(istype(H.glasses,/obj/item/clothing/glasses/meson) && current_size != 11)
					H << SPAN_NOTE("You look directly into The [src.name], good thing you had your protective eyewear on!")
					return
				else
					H << SPAN_WARN("You look directly into The [src.name], but your eyewear does absolutely nothing to protect you from it!")
		M << SPAN_DANG("You look directly into The [src.name] and feel [current_size == 11 ? "helpless" : "weak"].")
		M.apply_effect(3, STUN)
		M.visible_message(SPAN_DANG("[M] stares blankly at The [src]!"))

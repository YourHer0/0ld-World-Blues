			...
			if(buckled_mob.buckled == src)
				if(buckled_mob != user)
					buckled_mob.visible_message(\
						SPAN_NOTE("[user.name] frees [buckled_mob.name] from \the [src]."),\
						SPAN_NOTE("[user.name] frees you from \the [src]."),\
						"<span class='warning'>You hear shredding and ripping.</span>")
				else
					buckled_mob.visible_message(\
						SPAN_NOTE("[buckled_mob.name] struggles free of \the [src]."),\
						SPAN_NOTE("You untangle \the [src] from around yourself."),\
						"<span class='warning'>You hear shredding and ripping.</span>")
			unbuckle()
		else
			var/text = pick("rip","tear","pull")
			user.visible_message(\
				SPAN_NOTE("[user.name] [text]s at \the [src]."),\
				SPAN_NOTE("You [text] at \the [src]."),\
				"<span class='warning'>You hear shredding and ripping.</span>")
	return

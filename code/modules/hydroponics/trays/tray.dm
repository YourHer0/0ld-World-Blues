...
/obj/machinery/portable_atmospherics/hydroponics/proc/mutate_species()
	...
	update_icon()
	visible_message(
		SPAN_DANG("The " + SPAN_NOTE(previous_plant) + " has suddenly mutated into " + SPAN_NOTE(seed.display_name) + "!")
	)
...
/obj/machinery/portable_atmospherics/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/weapon/wirecutters) || istype(O, /obj/item/weapon/scalpel))
		...
		if(!seed)
			...
			user << SPAN_DANG("\The [src] already has seeds in it!")

	else if (istype(O, /obj/item/weapon/material/minihoe))  // The minihoe

		if(weedlevel > 0)
			user.visible_message(
				SPAN_DANG("[user] starts uprooting the weeds."),
				SPAN_DANG("You remove the weeds from the [src].")
			)
			weedlevel = 0
			update_icon()
		else
			user << SPAN_DANG("This plot is completely devoid of weeds. It doesn't need uprooting.")
		...
		if(seed)
			user << SPAN_DANG("[src] is already occupied!")
		else
		...
		user.visible_message(SPAN_DANG("\The [seed.display_name] has been attacked by [user] with \the [O]!"))
		if(!dead)
			health -= O.force
			check_health()
	return
...
/obj/machinery/portable_atmospherics/hydroponics/examine(mob/user, return_dist=1)
	...
	if(dead)
		usr << SPAN_DANG("The plant is dead.")

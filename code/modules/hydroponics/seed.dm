...
/datum/seed/proc/do_thorns(var/mob/living/carbon/human/target, var/obj/item/fruit, var/target_limb)
	...
	if(get_trait(TRAIT_CARNIVOROUS))
		if(get_trait(TRAIT_CARNIVOROUS) == 2)
			if(affecting)
				target << SPAN_DANG("\The [fruit]'s thorns pierce your [affecting.name] greedily!")
			else
				target << SPAN_DANG("\The [fruit]'s thorns pierce your flesh greedily!")
			damage = get_trait(TRAIT_POTENCY)/2
		else
			if(affecting)
				target << SPAN_DANG("\The [fruit]'s thorns dig deeply into your [affecting.name]!")
			else
				target << SPAN_DANG("\The [fruit]'s thorns dig deeply into your flesh!")
...
/datum/seed/proc/do_sting(var/mob/living/carbon/human/target, var/obj/item/fruit)
	...
	if(chems && chems.len)
		...
		target << SPAN_DANG("You are stung by \the [fruit]!")
		for(var/rid in chems)
...
/datum/seed/proc/thrown_at(var/obj/item/thrown,var/atom/target, var/force_explode)
	...
	if(force_explode || get_trait(TRAIT_EXPLOSIVE))
		...
		for(var/turf/T in valid_turfs)
			...
			splatter(T,thrown)
		if(origin_turf)
			origin_turf.visible_message(SPAN_DANG("The [thrown.name] explodes!"))
		qdel(thrown)
		return
	...
	if(get_trait(TRAIT_JUICY) && splatted)
		...
		if(origin_turf)
			origin_turf.visible_message(SPAN_DANG("The [thrown.name] splatters against [target]!"))
		qdel(thrown)
	...
	for(var/i = 0;i<total_mutations;i++)
		switch(rand(0,11))
			if(0) //Plant cancer!
				...
				source_turf.visible_message(SPAN_DANG("\The [display_name] withers rapidly!"))
			if(1)
...
/datum/seed/proc/harvest(var/mob/user,var/yield_mod,var/harvest_sample,var/force_amount)
	...
	if(!force_amount && get_trait(TRAIT_YIELD) == 0 && !harvest_sample)
		if(istype(user)) user << SPAN_DANG("You fail to harvest anything useful.")
	else

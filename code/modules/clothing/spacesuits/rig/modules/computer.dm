	...
	if(ai_card)
		if(istype(ai_card, /obj/item/device/aicard))
			if(integrated_ai && !integrated_ai.stat)
				if(user)
					user << SPAN_DANG("You cannot eject your currently stored AI. Purge it manually.")
				return 0
			user << SPAN_DANG("You purge the remaining scraps of data from your previous AI, freeing it for use.")
			if(integrated_ai)

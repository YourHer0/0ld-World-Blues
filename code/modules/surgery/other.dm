...
/datum/surgery_step/hardsuit
	...
	fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		user.visible_message(
			SPAN_DANG("[user]'s [tool] can't quite seem to get through the metal..."),
			SPAN_DANG("Your [tool] can't quite seem to get through the metal. It's weakening, though - try again.")
		)
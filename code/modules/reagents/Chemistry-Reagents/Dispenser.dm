...
/datum/reagent/acetone/touch_obj(var/obj/O)	//I copied this wholesale from ethanol and could likely be converted into a shared proc. ~Techhead
	...
	if(istype(O, /obj/item/weapon/book))
		...
		if(istype(O, /obj/item/weapon/book/tome))
			usr << SPAN_NOTE("The solution does nothing. Whatever this is, it isn't normal ink.")
			return
		var/obj/item/weapon/book/affectedbook = O
		affectedbook.dat = null
		usr << SPAN_NOTE("The solution dissolves the ink on the book.")
	return
		...
		for(var/ID in M.virus2)
			...
			if(prob(5))
				...
				if(prob(50))
					...
					var/absorbed = 0
					var/obj/item/organ/internal/nutrients/rad_organ = locate() in M.internal_organs
	...
	if(ishuman(M))
		...
		if(H.head)
			if(H.head.unacidable)
				H << SPAN_DANG("Your [H.head] protects you from the acid.")
				remove_self(volume)
				return
			else if(removed > meltdose)
				H << SPAN_DANG("Your [H.head] melts away!")
				qdel(H.head)
				H.update_inv_head(1)
				H.update_hair(1)
				removed -= meltdose
		...
		if(H.wear_mask)
			if(H.wear_mask.unacidable)
				H << SPAN_DANG("Your [H.wear_mask] protects you from the acid.")
				remove_self(volume)
				return
			else if(removed > meltdose)
				H << SPAN_DANG("Your [H.wear_mask] melts away!")
				qdel(H.wear_mask)
		...
		if(H.glasses)
			if(H.glasses.unacidable)
				H << SPAN_DANG("Your [H.glasses] partially protect you from the acid!")
				removed /= 2
			else if(removed > meltdose)
				H << SPAN_DANG("Your [H.glasses] melt away!")
				qdel(H.glasses)

		...
		if(ishuman(holder))
			...
			for(var/obj/item/organ/external/affecting in H.organs)
				if(card in affecting.implants)
					...
					H.visible_message(SPAN_DANG("\The [src] explodes out of \the [H]'s [affecting.name] in shower of gore!"))
					break
...
/mob/living/silicon/pai/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(W.force)
		visible_message(SPAN_DANG("[user.name] attacks [src] with [W]!"))
...
/mob/living/silicon/pai/attack_hand(mob/user as mob)
	visible_message(SPAN_DANG("[user.name] boops [src] on the head."))
	close_up()

...
/mob/living/carbon/relaymove(var/mob/living/user, direction)
	if((user in src.stomach_contents) && istype(user))
		if(user.last_special <= world.time)
			...
			src.visible_message(SPAN_DANG("You hear something rumbling inside [src]'s stomach..."))
			...
			if(I && I.force)
				...
				user.visible_message(SPAN_DANG("[user] attacks [src]'s stomach wall with the [I.name]!"))
		...
		if(src == M && ishuman(src))
			...
			if (M.on_fire)
				...
				if(do_mob(M, src, 15))
					...
					if (M.on_fire)
						M.visible_message(
							SPAN_DANG("The fire spreads from [src] to [M]!"),
							SPAN_DANG("The fire spreads to you as well!")
						)
					else

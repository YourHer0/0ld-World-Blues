...
/mob/living/carbon/human/proc/handle_grasp()
	...
	if(l_hand)
		for(var/limb_tag in list(BP_L_HAND,BP_L_ARM))
			...
			if(!E)
				visible_message(SPAN_DANG("Lacking a functioning left hand, \the [src] drops \the [l_hand]."))
	...
	if(r_hand)
		for(var/limb_tag in list(BP_R_HAND,BP_R_ARM))
			var/obj/item/organ/external/E = get_organ(limb_tag)
			if(!E)
				visible_message(SPAN_DANG("Lacking a functioning right hand, \the [src] drops \the [r_hand]."))
